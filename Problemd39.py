...
perimeter_ways = [0] * (5*10**6 + 1) # we find all the perimeters upto N(p<=N) so that we dont have to recompute for each test cases
                                     # perimeter ways stores the no of ways to make each perimeters and each index number is the perimeter itself

for m in range(1, 1581): #as perimeter should be less or equal to 5000000, the 2m^2 term should be less than 5000000 meaning m<1581
    ...
    for n in range(1, m): 
        if (m + n) % 2 and gcd(m, n) == 1: #checking if of opposite parity and coprimer
            p = 2 * m * (m + n) 
            if p > 5000000: #though we found m value for p<=5000000, still adding up n might lead to p>5000000. so handling that edge case
                continue
            perimiter_ways[p] += 1
            i = 2
            while i * p <= 5000000: 
...
max_perimeter_upto = [0] * (5*10**6 + 1) # this array stores the perimeter having the maximum no of ways among the perimeters upto given index number


for perimeter in range(1, 5000 * 1000 + 1):
  
    max_perimeter = max_perimeter_upto[i-1] #We take the current perimeter with the maximum no of ways

    # and then compare it to the the no of ways of the next perimeter
    if perimeter_ways[perimeter] > perimeter_ways[max_perime]: #if that next perimeter has more no of ways than the current perimeter with max no of ways
